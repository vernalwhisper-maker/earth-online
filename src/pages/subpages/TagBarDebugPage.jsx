...
import { DEBUG_DEFAULTS, MODE_OPTIONS, STORAGE_KEY_TAGBAR } from "../../config/debugDefaults";

function loadParams() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY_TAGBAR);
    if (!raw) return null;
...
function saveParams(params) {
  localStorage.setItem(STORAGE_KEY_TAGBAR, JSON.stringify(params));
}
...
export default function TagBarDebugPage({ onBack }) {
  ...
  useEffect(() => {
    ...
    saveParams(params);
    window.dispatchEvent(new CustomEvent("earth-debug-tagbar-changed", { detail: params }));
  }, [elasticity, blurAmount, saturation, displacementScale, aberrationIntensity, cornerRadius, modeIdx, overLight, shadowOpacity]);

  const handleReset = () => {
    localStorage.removeItem(STORAGE_KEY_TAGBAR);
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
      ...
      <section className="bg-surface rounded-card border border-scribe p-4 mb-4 overflow-hidden">
        ...
        <div className="flex items-center justify-center py-6 rounded-xl"
          style={{ background: `url("/测试专用.png") center/cover no-repeat, ${isDarkPreview ? "#1c1b1a" : "#f0efec"}`, minHeight: 80 }}>
          <LiquidGlass
            cornerRadius={cornerRadius} padding="4px 8px"
