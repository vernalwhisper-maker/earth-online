    ...
    <nav className="fixed bottom-5 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] max-w-md">
      <div className="relative flex items-center justify-around rounded-[1.5rem] px-4 py-1 shadow-[0_8px_32px_rgba(0,0,0,0.12)] safe-area-bottom overflow-hidden">
        <div className="absolute inset-0 backdrop-blur-xl"
          style={{ background: "linear-gradient(to bottom, rgba(255,255,255," + (tabBarOpacity / 100) + "), rgba(255,255,255," + (tabBarOpacity / 200) + "))" }} />
            ...
            return (
              <button key={tab.key} onClick={() => onNavigate(tab.key)}
                ...
                <div className="relative">
                  ...
                  {/* Pending todo badge */}
                  {showTodoBadge && (
                    <span className="absolute -top-1.5 -right-2.5 bg-blue-500 text-white text-[10px] font-mono font-semibold rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1">
                      {activeTodoCount}
                    </span>
                  )}
